...
export const judgeStreams = mysqlTable("judge_streams", {
  ...
  // pending = judge requested a seat; approved = producer cleared the seat;
  // active = camera and microphone have successfully published.
  status: mysqlEnum("status", ["pending", "approved", "active", "ended", "error"]).default("pending").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  endedAt: timestamp("endedAt"),
});
...
export const beatProducerDirectPaymentMethods = mysqlTable("beat_producer_direct_payment_methods", {
  ...
  provider: mysqlEnum("provider", ["cashapp", "paypal", "zelle", "venmo", "apple_pay", "chime", "other"]).notNull(),
...
export const marketplaceBeats = mysqlTable("marketplace_beats", {
  ...
  previewTagSource: mysqlEnum("previewTagSource", ["none", "purchase_now", "purchase_today", "mitten", "custom"]).default("none").notNull(),
  previewTagFileKey: varchar("previewTagFileKey", { length: 512 }),
  previewTagFileUrl: varchar("previewTagFileUrl", { length: 512 }),
  previewTagAtSeconds: int("previewTagAtSeconds").default(0).notNull(),
...
export const beatSales = mysqlTable("beat_sales", {
  ...
  beatTitleSnapshot: varchar("beatTitleSnapshot", { length: 512 }).notNull(),
  ...
  stripePaymentIntentId: varchar("stripePaymentIntentId", { length: 256 }),
  stripeBalanceTransactionId: varchar("stripeBalanceTransactionId", { length: 256 }),
  status: mysqlEnum("status", ["pending", "paid", "refunded", "disputed"]).default("pending").notNull(),
  producerEarningsStatus: mysqlEnum("producerEarningsStatus", ["pending", "available", "reversed"]).default("pending").notNull(),
  producerEarningsAvailableAt: timestamp("producerEarningsAvailableAt"),
  producerEarningsSettledAt: timestamp("producerEarningsSettledAt"),
...
export const beatDirectPayments = mysqlTable("beat_direct_payments", {
  ...
  confirmationMode: mysqlEnum("confirmationMode", ["producer", "admin"]).default("producer").notNull(),
