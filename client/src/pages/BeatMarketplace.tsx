        ...
        <section className="relative border-b border-white/10 bg-[#090909]">
          ...
          <div className="container relative grid gap-12 py-12 md:grid-cols-[1.18fr_.82fr] md:items-end md:py-20 lg:gap-16">
            ...
            <div className="border border-red-500/35 bg-black/55 p-5 shadow-[0_0_60px_rgba(120,0,0,.12)] backdrop-blur-sm md:p-6">
              <div className="mb-5 flex items-center justify-between border-b border-white/10 pb-4"><div><p className="text-[10px] font-black uppercase tracking-[.24em] text-red-400">The move</p><h2 className="mt-1 font-['Anton'] text-3xl uppercase">Put your sound in the room</h2></div><Sparkles className="h-7 w-7 text-yellow-300" /></div>
              <div className="grid gap-3 sm:grid-cols-3 md:grid-cols-1 lg:grid-cols-3">
                {[{ number: "01", title: "Upload", text: "Build your catalog" }, { number: "02", title: "Get discovered", text: "Let artists find your sound" }, { number: "03", title: "Sell / license", text: "Clear terms, real delivery" }].map((step) => <div key={step.number} className="border border-white/10 bg-[#111] p-4"><span className="text-xs font-black text-red-500">{step.number}</span><h3 className="mt-4 text-sm font-black uppercase">{step.title}</h3><p className="mt-1 text-xs leading-relaxed text-white/45">{step.text}</p></div>)}
              </div>
              <div className="mt-5 flex items-center gap-3 border-t border-white/10 pt-5"><div className="grid h-10 w-10 place-items-center rounded-full bg-yellow-400 text-black"><Check className="h-5 w-5" /></div><div><p className="text-sm font-black uppercase">100% royalties with Pro</p><p className="text-xs text-white/45">Keep the full marketplace share on your licenses.</p></div></div>
            </div>
          </div>
        </section>

        <section className="container border-b border-white/10 py-14 md:py-20">
          <div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr] lg:items-start"><div><p className="text-[10px] font-black uppercase tracking-[.28em] text-red-500">Why this exists</p><h2 className="mt-3 max-w-lg font-['Anton'] text-5xl uppercase leading-[.9]">Your beats.<br />Your catalog.<br /><span className="text-red-600">Your royalties.</span></h2></div><div className="grid gap-3 sm:grid-cols-2"><p className="border-l-2 border-red-600 pl-4 text-sm leading-relaxed text-white/65">Murder Mitten Media is building a real home for Michigan producers to showcase their sound and get found by artists.</p><div className="grid grid-cols-2 gap-2 text-xs font-bold uppercase tracking-wide text-white/60">{["Upload beats", "Build your profile", "Organize your catalog", "Set license terms", "Get discovered", "Keep 100% with Pro"].map((item) => <div key={item} className="border border-white/10 bg-white/[.03] p-3">{item}</div>)}</div></div></div>
        </section>

        <section className="container py-14 md:py-20"><div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="text-[10px] font-black uppercase tracking-[.28em] text-red-500">How it works</p><h2 className="mt-2 font-['Anton'] text-5xl uppercase">Make your move</h2></div><p className="max-w-sm text-sm leading-relaxed text-white/45">No complicated industry language. Set up your presence, upload your sound, and let the catalog work.</p></div><div className="grid gap-px bg-white/10 md:grid-cols-3">{[{ n: "01", title: "Create your producer profile", text: "Set up your producer presence on Murder Mitten Media." }, { n: "02", title: "Upload your beats", text: "Add audio, artwork, pricing, and simple licensing information." }, { n: "03", title: "Get discovered", text: "Artists browse the marketplace and find the sound they need. Free claim or secure checkout." }].map((step) => <article key={step.n} className="bg-[#101010] p-6 md:p-8"><span className="font-['Anton'] text-5xl text-red-600">{step.n}</span><h3 className="mt-8 max-w-xs font-['Anton'] text-2xl uppercase leading-none">{step.title}</h3><p className="mt-4 max-w-xs text-sm leading-relaxed text-white/50">{step.text}</p></article>)}</div></section>
