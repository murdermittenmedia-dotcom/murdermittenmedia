  ...
  return (
    <div className={`relative overflow-hidden bg-gradient-to-br from-red-950 via-zinc-900 to-black ${large ? "aspect-square" : "aspect-[4/3]"}`}>
      ...
      <div className="absolute bottom-3 right-3">
        {beat.youtubeUrl ? <YouTubePreviewButton videoId={beat.youtubeUrl.match(/[?&]v=([^&]+)/)?.[1] || ""} title={beat.title} className={large ? "h-11 w-11" : ""} /> : <AudioPlayButton url={beat.previewFileUrl} title={beat.title} artist={beat.producerName} size={large ? "lg" : "md"} className="border border-white/25 bg-red-600 hover:bg-red-500" />}
      </div>
    </div>
  );
}
    ...
    <article className="group overflow-hidden border border-white/10 bg-[#101010] transition duration-200 hover:-translate-y-1 hover:border-red-500/50">
      ...
      <div className="space-y-3 p-4">
        <div className="min-w-0">
          <Link href={`/beats/${beat.slug}`} title={beat.title} className="block line-clamp-3 font-['Anton'] text-xl uppercase tracking-wide text-white hover:text-red-400">{beat.title}</Link>
